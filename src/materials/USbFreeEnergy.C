...
registerMooseObject("BeaverApp", USbFreeEnergy);

template <>
InputParameters
validParams<USbFreeEnergy>()
{
  ...
  params.addRequiredCoupledVar("c", "Concentration variable of U");
...
USbFreeEnergy::USbFreeEnergy(const InputParameters & parameters)
  : DerivativeParsedMaterialHelper(parameters),
    _c("c"),//U concentration for now, can it change to Sb?
    ...
    _m(getParam<Real>("unit_conversion"))
{
  ...
  if (_stoi == "U4Sb3")
  {
    Real a = -5.0273e5;
    Real b = 2.28e1;
    free_energy(_c) = 1.0/7* _m * (g_offset + 4.0*G_U
                        + 3.0*G_Sb 
                        + a + b * _T + _k * pow(_c - 4.0/7,2)); // last term to penalize any deviation from the intermetallic composition U4Sb3
    std::cout << "Free_energy of" << _stoi << " is " << 1.0/7* (g_offset+4.0*G_U + 3.0*G_Sb + a + b * _T ) << std::endl; //print the free_energy in console, the unit is J/mol, keep this for test
  }
  ...
  else if (_stoi == "U3Sb4")
  {
    Real a = -4.832e5;
    Real b = 0.6;
    free_energy(_c) = 1.0/7* _m * (g_offset + 3.0*G_U
                        + 4.0*G_Sb
                        + a + b * _T + _k * pow(_c - 3.0/7,2));
  }
  ...
  else if (_stoi == "USb")
  {
    Real a = -1.4e5;
    Real b = 0;
    free_energy(_c) = 1.0/2* _m * (G_U
                        + G_Sb 
                        + a + b * _T + _k * pow(_c - 0.5,2));
    std::cout << "Free_energy of" << _stoi << " is " << 1.0/7* (g_offset+4.0*G_U + 3.0*G_Sb + a + b * _T ) << std::endl;
  }
  ...
  else if (_stoi == "USb2")
  {
    Real a = -1.803e5;
    Real b = 2.01;
    free_energy(_c) = 1.0/3* _m * (G_U
                        + 2.0*G_Sb 
                        + a + b * _T + _k * pow(_c - 1.0/3,2));
  }
  ...
  { free_energy(_c) = _m * (G_U+ _k * pow(_c - 1,2));
  }
